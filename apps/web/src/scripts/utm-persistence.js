 ...
 *
 * Behavior:
 * - If any utm_* param is present in the URL, that exact set is stored for the
 *   session (URL always wins; last touch within the session) and nothing else
 *   happens — ST reads the URL directly.
 ...
 *
 * Storage is sessionStorage: attribution covers the click-through session only,
 * never later direct visits. All failures are swallowed — attribution must
...
(function () {
  ...
  function writeStoreFromUrl(params) {
    ...
    if (!has) return false;
    try {
      window.sessionStorage.setItem(STORE_KEY, JSON.stringify(out));
    } catch (e) {
